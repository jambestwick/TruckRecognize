...
import com.kernal.plateid.utills.FrameCapture;
...
public class MemoryCameraActivity extends Activity implements
	...
	private static final String TAG = "MemoryCameraActivity";
	...
	int uiRot;
 	public ServiceConnection recogConn = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			Log.e("MemoryCameraActivity", "进入ServiceConnected");

			recogBinder = (RecogService.MyBinder) service;
			iInitPlateIDSDK = recogBinder.getInitPlateIDSDK();

			Log.e("MemoryCameraActivity.", "onServiceConnected.iInitPlateIDSDK=" + iInitPlateIDSDK);
...
//parameter2:6，0,1
			Log.e("MemoryCameraActivity", "before.recogBinder.getnRet1()=" + recogBinder.getnRet());
			recogBinder.setRecogArgu(cfgparameter, imageformat, bVertFlip,
					bDwordAligned);
			Log.e("MemoryCameraActivity", "recogBinder.getnRet1()=" + recogBinder.getnRet());
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.e("MemoryCameraActivity", "isCamera=" + isCamera);
		Log.e("MemoryCameraActivity", "cameraRecogUtill=" + cameraRecogUtill);
		if (isCamera) {
			if (cameraRecogUtill) {
	...
	private void findiew() {
		...
		take_pic.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				Log.e("MemeoryCameraActivity", "take_pic被单击");
				isCamera = true;
			}
	...
	{
		// 实时监听屏幕旋转角度
		Log.e("MemoryCameraActivity", "进入onPreViewFrame");
		...
		if (iInitPlateIDSDK == 0) {
			...
			if (cameraRecogUtill) {
				...
				Log.e("MemoryCameraActivity", "进入拍照识别模式");
				// 拍照识别 在使用根据图片路径识别时 执行下列代码
				if (isCamera) {

					Log.e("MemoryCameraActivity", "拍照按钮点击后的执行代码");
					...
					Log.e("MemoryCameraActivity", "onPreviewFrame.nRet=" + nRet);
					if (nRet != 0) {
				...
				prp.picByte = data;
				Log.e("MemoryCameraActivity", "data=" + data);
				picData = data;		
				if (rotation == 0) {
					// 通知识别核心,识别前图像应先旋转的角度
				...
				if (isCamera) {
					...
					Log.e("MemoryCameraActivity", "视频流识别模式，isCamera=True");
					if(myview.length!=0&&surfaceView.getHeight()!=0){
//						System.out.println("视频流识别模式");
						Log.e("MemoryCameraActivity", "before.recogBinder.getnRet2()=" + recogBinder.getnRet());
						fieldvalue = recogBinder.doRecogDetail(prp);

						Log.e("MemoryActivity", "进入获取filedValue的方法fieldvalue=" + Arrays.asList(fieldvalue));
						Log.e("MemoryCameraActivity", "recogBinder.getnRet2()=" + recogBinder.getnRet());
					}
					nRet = recogBinder.getnRet();
//					nRet=0;

					Log.e("MemoryCameraActivity", "视频流识别模式，nRet=" + nRet);
					Log.e("MemoryCameraActivity", "recogBinder.getnRet3()=" + recogBinder.getnRet());
					if (nRet != 0) {
						String[] str = { "" + nRet };
						getResult(str);
					} else {
...
//									System.out.println("保存图片-------------");
								...
								intent.putExtra("recogType", recogType);
								Log.e("MemoryCameraActivity", "检测到车牌");
								MemoryCameraActivity.this.finish();
								startActivity(intent);
	...
	private void feedbackWrongCode() {
		...
		Log.e("MemoryCameraActiviyt", "nretString=" + nretString);
